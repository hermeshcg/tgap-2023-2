  ...
  shortestDistance(initialVertex) {
    ...
    while(currentVertex) {
      ...
      for(let e of edges) {
        ...
        if(!result[e.adjacent].isClosed && result[e.adjacent].distance < closer.distance) {
          closer.vertex = e.adjacent
          closer.distance = result[e.adjacent].distance
        }

      }

      // Determina o próximo vértice a ser analisado
      currentVertex = closer.vertex

      // Se o vértice adjacente mais curto for válido, fecha-o
      if(closer.vertex) result[closer.vertex].isClosed = true
      
      // Se não houver nenhum vértice adjacente ainda aberto, faz uma busca na workTable
      // para verificar se há algum vértice ainda aberto, reiniciando a análise por ele
      else for(let vtx in result) {
        if(!result[vtx].isClosed) {
          currentVertex = vtx
          break
        }
      }
